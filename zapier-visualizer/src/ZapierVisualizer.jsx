...
// ---------------------------------------------------------------------------
// Node-type semantic map — each type now occupies its own palette lane so
// a flow reads as four distinct stages, not four shades of the same pastel:
//   TRIGGER (entry / spark)   → Thistle pastel on Deep Thistle  (warm-cool)
//   FILTER  (gate / decision) → Cool ash on Deep Thistle border (caution signal)
//   ACTION  (busy / working)  → Little Boy Blue on Liberty Blue (the "doing" blue)
//   FINAL   (terminus)        → solid Navy Ink with white text  (flow anchor)
// Each entry carries its own text color so the Navy-Ink card stays readable.
// ---------------------------------------------------------------------------
const NODE_STYLES = {
  trigger: {
    fill: COLORS.slateBlue,         // Thistle pastel
    border: COLORS.criticalText,    // Deep Thistle (#8C5E7F)
    text: COLORS.text,              // Navy Ink on pastel
    label: "TRIGGER",
  },
  filter: {
    fill: COLORS.cardBgMuted,       // Cool ash
    border: COLORS.criticalText,    // Deep Thistle border — gate = caution
    text: COLORS.text,
    label: "FILTER",
  },
  action: {
    fill: COLORS.sage,              // Little Boy Blue pastel
    border: COLORS.accent,          // Liberty Blue
    text: COLORS.text,
    label: "ACTION",
  },
  end: {
    fill: COLORS.text,              // Solid Navy Ink — anchors the flow
    border: COLORS.text,
    text: COLORS.cardBg,            // Silk White text on Navy Ink
    label: "FINAL",
  },
};

// App color palette — reharmonized into the Heritage Silver cool register.
// Every swatch is distinct from its neighbors, lives in the navy → petrol →
// thistle → plum family (no warm oranges/reds/greens), and meets ~4.5:1 WCAG
// AA contrast with white text on the 24px badges.
const APP_COLORS = {
  Typeform: "#2A3547",          // Navy Ink
  HubSpot: "#8C5E7F",           // Deep Thistle (brand lean: warmth without orange)
  Gmail: "#7A3E4E",             // Deep rose-plum
  Asana: "#9B637D",             // Muted mauve
  Slack: "#5B4D75",             // Deep thistle-purple
  "Google Calendar": "#3B5678", // Mid petrol blue
  Calendly: "#4A6A94",          // Liberty petrol (slightly brighter than GCal)
  "Google Sheets": "#3F6A5E",   // Deep slate-teal (cool-register stand-in for green)
  "Google Drive": "#566175",    // Mid slate
  "Claude AI": "#54678F",       // Deep Liberty Blue (AA on white)
  Zapier: "#7C6E9C",            // Mid thistle-purple (replaces the warm Zapier orange)
  Notion: "#3B4558",            // Dark charcoal-slate
};
...
// ---------------------------------------------------------------------------
// Complexity tier encoding — three distinct palette positions so the chip
// carries more information than a binary "hard/easy" split. Text is always
// Navy Ink for readability on all three pastels.
// ---------------------------------------------------------------------------
const COMPLEXITY_STYLES = {
  ...
  Intermediate: {
    fill: COLORS.slateBlue,          // Thistle pastel
    border: COLORS.slateBlueBorder,
  },
  Advanced: {
    fill: COLORS.accentDim,          // Dimmed Liberty Blue
    border: COLORS.accent,           // Full Liberty Blue
  },
};

function complexityStyle(level) {
  return COMPLEXITY_STYLES[level] || COMPLEXITY_STYLES.Intermediate;
}

// ---------------------------------------------------------------------------
// Per-workflow accent — each workflow gets its own palette-lane color so the
// active-tab + workflow-overview card both carry a distinct tint. Lets the
// user *feel* which workflow they're in without reading the title.
// ---------------------------------------------------------------------------
const WORKFLOW_ACCENTS = {
  "lead-intake":          { solid: "#4A6A94", pastel: COLORS.sage,       pastelBorder: COLORS.sageBorder       }, // Little Boy Blue
  "calendar-guard":       { solid: "#8C5E7F", pastel: COLORS.critical,   pastelBorder: COLORS.criticalBorder   }, // Deep Thistle
  "expense-automation":   { solid: "#6B7FAB", pastel: COLORS.accentDim,  pastelBorder: COLORS.accent           }, // Liberty Blue
  "weekly-briefing":      { solid: "#7C6E9C", pastel: COLORS.slateBlue,  pastelBorder: COLORS.slateBlueBorder  }, // Thistle
};

function workflowAccent(id) {
  return (
    WORKFLOW_ACCENTS[id] || {
      solid: COLORS.accent,
      pastel: COLORS.accentDim,
      pastelBorder: COLORS.accent,
    }
  );
}

// ---------------------------------------------------------------------------
// Tiny hex→rgba helper for tint backgrounds and borders. Accepts 6-digit hex.
// ---------------------------------------------------------------------------
...
function FlowNode({ node, index, selected, onClick }) {
  ...
  const { fill, border, text: nodeText = COLORS.text, label } = nodeStyle(node.type);
  const isDarkCard = fill === COLORS.text;        // FINAL = solid Navy Ink
  const chipFill = isDarkCard ? COLORS.cardBg : COLORS.cardBg; // white chip works on both
  const chipTextColor = COLORS.text;              // chip itself stays light; text Navy Ink
...
function NodeDetail({ node, workflow, onClose }) {
  ...
  const wa = workflowAccent(workflow?.id);
  return (
    <div
      className="zv-card"
      style={{
        background: COLORS.cardBg,
        border: `1px solid ${COLORS.borderColor}`,
        borderLeft: `3px solid ${wa.solid}`,
        borderRadius: 12,
        // Same per-workflow top stripe as the overview card, so whichever
        // workflow you're inside has a consistent tint across both views.
        backgroundImage: `linear-gradient(to bottom, ${wa.solid} 0, ${wa.solid} 4px, ${COLORS.cardBg} 4px)`,
    ...
    >
      ...
      {/* Data used — wrapped in a Thistle-tinted callout so the section reads
          as its own layer (parallel to Reliability, Metric grid, Impl note). */}
      <div
        style={{
          background: COLORS.slateBlue,                // Thistle pastel
          borderRadius: 10,
          padding: "12px 14px",
          border: `1px solid ${COLORS.slateBlueBorder}`,
          borderLeft: `3px solid ${COLORS.slateBlueText}`,
        }}
      >
        <div
          style={{
            fontSize: 10,
            color: COLORS.slateBlueText,
            fontFamily: FONT_STACK,
            letterSpacing: 1,
            marginBottom: 8,
            textTransform: "uppercase",
          ...
          {node.dataUsed.map((d, i) => (
            <span
              ...
              style={{
                ...
                borderRadius: 50,
                fontSize: 11,
                background: COLORS.cardBg,             // white pills pop on Thistle
                color: COLORS.text,
                border: `1px solid ${COLORS.borderColor}`,
                fontFamily: FONT_STACK,
                fontWeight: 500,
      ...
      {/* Implementation note — Deep Thistle stripe so this "ask-me-more"
          context reads as its own register, not another Liberty-Blue section. */}
      <div
        style={{
          background: COLORS.cardBgMuted,
          borderRadius: 10,
          padding: "12px 14px",
          border: `1px solid ${COLORS.borderColor}`,
          borderLeft: `3px solid ${COLORS.criticalText}`,
        }}
      >
        <div
          style={{
...
function WorkflowOverview({ workflow }) {
  const wa = workflowAccent(workflow.id);
  const cs = complexityStyle(workflow.complexity);
  return (
    <div
      className="zv-card"
      style={{
        ...
        borderLeft: `3px solid ${wa.solid}`,
        borderRadius: 12,
        // Accent stripe + complexity echo, stacked at the top of the card.
        // The per-workflow color identifies the workflow; the complexity color
        // below it signals difficulty at a glance.
        backgroundImage: `linear-gradient(to bottom, ${wa.solid} 0, ${wa.solid} 4px, ${cs.border} 4px, ${cs.border} 6px, ${COLORS.cardBg} 6px)`,
        paddingTop: 28,
        padding: "28px 24px 24px",
        display: "flex",
        flexDirection: "column",
    ...
    >
            ...
            return (
              <span
                ...
                style={{
                  ...
                  borderRadius: 50,
                  fontSize: 11,
                  // Per-app tint: 12% fill + 38% border. Navy Ink text stays ≥ 4.5:1 on
                  // every tint because the pastels all sit on Silk White body behind.
                  background: hexToRgba(ac, 0.12),
                  color: COLORS.text,
                  border: `1px solid ${hexToRgba(ac, 0.38)}`,
                  fontFamily: FONT_STACK,
                  fontWeight: 600,
                }}
              >
      ...
      {/* Reliability — three distinct pastel-tinted callouts. Same visual
          grammar as EA Control Center's BLOCKED / WAITING / FOLLOW-UP boxes. */}
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[
              {
                label: "Retries",
                value: workflow.reliability.retries,
                fill: COLORS.sage,                    // Little Boy Blue — "in progress / resilience"
                border: COLORS.sageBorder,
                accent: COLORS.sageText,
              },
              {
                label: "Failures",
                value: workflow.reliability.failures,
                fill: COLORS.critical,                // Deep Thistle pastel — caution
                border: COLORS.criticalBorder,
                accent: COLORS.criticalText,
              },
              {
                label: "Monitoring",
                value: workflow.reliability.monitoring,
                fill: COLORS.slateBlue,               // Thistle pastel — informational
                border: COLORS.slateBlueBorder,
                accent: COLORS.slateBlueText,
              },
            ].map(({ label, value, fill, border, accent }) => (
              <div
                key={label}
                style={{
                  background: fill,
          ...
          {WORKFLOWS.map((w) => {
            ...
            return (
              <button
                ...
                style={{
                  // Active tab picks up the workflow's own accent so the
                  // selection's color carries through to the overview card.
                  background: isActive ? wa.pastel : "transparent",
                  color: isActive ? COLORS.text : COLORS.textSecondary,
                  border: isActive
                    ? `1px solid ${wa.pastelBorder}`
